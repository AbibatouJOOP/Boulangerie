...
class PaiementRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        $rules = [
            ...
            'montant_payé' => 'required|numeric|min:0|max:10000000',
            'date_paiement' => 'required|date',
            'statut' => 'required|in:payée,non_payée',
            'numero_telephone' => 'nullable|string|max:20|regex:/^(\+221)?[0-9\s\-\(\)]{8,}$/',
            'operateur' => 'nullable|in:orange,mtn,moov',
        ...
        if ($this->isMethod('PUT') || $this->isMethod('PATCH')) {
            ...
            $rules['montant_payé'] = 'sometimes|numeric|min:0|max:10000000';
            $rules['date_paiement'] = 'sometimes|date';
            $rules['statut'] = 'sometimes|in:payée,non_payée';
    ...
    public function messages(): array
    {
        return [
            ...
            'operateur.in' => 'L\'opérateur doit être orange, mtn ou moov',
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            if ($this->isMethod('POST') && $this->commande_id) {
                $paiementExistant = \App\Models\Paiements::where('commande_id', $this->commande_id)->exists();
                if ($paiementExistant) {
            ...
            if ($this->commande_id && $this->montant_payé) {
                $commande = \App\Models\Commandes::find($this->commande_id);
                if ($commande && $this->montant_payé > $commande->montant_total * 1.1) { // Tolérance de 10%
                    $validator->errors()->add('montant_payé', 'Le montant payé ne peut pas dépasser significativement le total de la commande');
                }
            }
        });
    }
